...
def generate_launch_description():
    ...
    world_path = os.path.join(
        get_package_share_directory('cyberknife'),
        'worlds',
        'empty_world.world'
    )
    ...
    spawn_robot_world = IncludeLaunchDescription(
        PythonLaunchDescriptionSource(
            os.path.join(get_package_share_directory('cyberknife'), 'launch',
                         'spawn_cyberknife_vel.launch.py'),
        )
    )
    ...
    #         ' topic pub /joint_velocities/commands std_msgs/msg/Float64MultiArray "{data: [0.0, 0.0, 1.57, 0.0, 0.0, 0.0],layout: {dim:[], data_offset: 1"}} '
    #     ]],
    #     shell = True
    # )
